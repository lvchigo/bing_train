...
static inline int findFromList(const T &word, const vector<T> &strList) 
{
	int idx = -1;
	string tmp;
	map< string, int > mapStrList;
	map< string, int >::iterator itStrList;

	//printf("strList size:%d,strList:",strList.size());
	for (int i=0;i<strList.size();i++)
	{
		tmp = strList[i];
		mapStrList[tmp] = i;
		//printf("%d-%s ",i,tmp.c_str());
	...
	//find
	itStrList = mapStrList.find(string(word));
	if (itStrList != mapStrList.end()) // not find
	{
		idx = itStrList->second;
	}

	//printf("[findFromList]word:%s,idx:%d\n",string(word).c_str(),idx);
