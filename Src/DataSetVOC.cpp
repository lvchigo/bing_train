...
/*	printf("trainfile:%sImageSets/Main/train.txt,trainNum:%d,trainSet:",wkDir.c_str(),trainNum);
	for (int i=0;i<trainSet.size();i++)
		printf("%d-%s ",i,trainSet[i].c_str());
	printf("\n");

	printf("testfile:%sImageSets/Main/test.txt,testNum:%d,testSet:",wkDir.c_str(),testNum);
	for (int i=0;i<testSet.size();i++)
		printf("%d-%s ",i,testSet[i].c_str());
	printf("\n");*/

	printf("classNamesFile:%sImageSets/Main/class.txt,classNamesNum:%d,classNames:",wkDir.c_str(),classNames.size());
	for (int i=0;i<classNames.size();i++)
		printf("%d-%s ",i,classNames[i].c_str());
	printf("\n");
...
void DataSetVOC::loadAnnotations()
{
	...
	{
		res = false;
		if ( Bin_Xml_Yml == 1 )	//Bin_Xml_Yml:1-Xml,0-Yml;
			res = loadBBoxes_XML(trainSet[i], gtTrainBoxes[i], gtTrainClsIdx[i]);
		else if ( Bin_Xml_Yml == 0 )	//Bin_Xml_Yml:1-Xml,0-Yml;
			res = loadBBoxes(trainSet[i], gtTrainBoxes[i], gtTrainClsIdx[i]);
		if (!res)
			return;
	}
	...
	for (i = 0; i < testNum; i++)
	{
		res = false;
		if ( Bin_Xml_Yml == 1 )	//Bin_Xml_Yml:1-Xml,0-Yml;
			res = loadBBoxes_XML(testSet[i], gtTestBoxes[i], gtTestClsIdx[i]);
		else if ( Bin_Xml_Yml == 0 )	//Bin_Xml_Yml:1-Xml,0-Yml;
			res = loadBBoxes(testSet[i], gtTestBoxes[i], gtTestClsIdx[i]);
		if (!res)
			return;
	}
...
void DataSetVOC::loadBox(const FileNode &fn, vector<Vec4i> &boxes, vecI &clsIdx){
	...
	//printf("bndbox:%s,%s,%s,%s\n",strXmin.c_str(),strYmin.c_str(),strXmax.c_str(),strYmax.c_str());
	//printf("clsName:%s,clsIdx:%d\n",clsName.c_str(),findFromList(clsName, classNames));
...
bool DataSetVOC::loadBBoxes(CStr &nameNE, vector<Vec4i> &boxes, vecI &clsIdx)
{
	...
	//printf("fName:%s,annoPathW:%s,nameNE:%s\n",fName.c_str(),annoPathW.c_str(),nameNE.c_str());
...
bool DataSetVOC::loadBBoxes_XML(CStr &nameNE, vector<Vec4i> &boxes, vecI &clsIdx)
{
	...
	string fName = format(_S(annoPathW), _S(nameNE));
	//printf("fName:%s,annoPathW:%s,nameNE:%s\n",fName.c_str(),annoPathW.c_str(),nameNE.c_str());
	int nRet = api_xml.load_xml( fName, xmlImageID, vecXmlLabelRect );
	if ( (nRet!=0) || (vecXmlLabelRect.size()<1) )
	{
		cout<<"[loadXml Err!!loadXml:]"<<fName;
		return false;
	}
	...
	boxes.clear();
	clsIdx.clear();
	for (int i=0;i<vecXmlLabelRect.size();i++)
	{
		boxes.push_back(vecXmlLabelRect[i].second);
		clsIdx.push_back(findFromList(vecXmlLabelRect[i].first, classNames));	
	}
